...
/**
 * Applica (www.applicadoit.com) User: bimbobruno Date: 08/10/14 Time: 15:24
 */
/**
...
public class DefaultRepositoriesFactory implements RepositoriesFactory {
    ...
    public Repository createForEntity(Class<? extends Entity> type) {
        ...
        try {
            repository = applicationContext.getBeansOfType(Repository.class).
                    values().stream()
                    .filter(r -> type.equals(r.getEntityType()))
                    .findFirst()
                    .orElseThrow(() -> new NoSuchBeanDefinitionException(
                                    "repository " + type.getName()));
        } catch (NoSuchBeanDefinitionException e) {
            repository = (DefaultRepository) applicationContext.getBean(
                    "default-repository");
